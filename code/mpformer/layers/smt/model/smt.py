...
from mmengine.model import constant_init, trunc_normal_init, BaseModule, ModuleList, Sequential
...
class Block(BaseModule):
    ...
    def __init__(self, 
                 ...
                 ca_attention=1,
                 expand_ratio=2,
                 use_convnext_path=True,
                 init_cfg=None):
        super().__init__()

        self.init_cfg = init_cfg
        self.use_convnext_path = use_convnext_path
        ...
        # 定义局部卷积路径（仅当启用时初始化）
        if use_convnext_path:
            self.convnext_path = nn.Sequential(
    ...
    def forward(self, x, H, W, stage_id):
        x = x + self.drop_path(self.attn(self.norm1(x), H, W))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
        
        # 如果启用 ConvNeXt 风格的卷积路径
        if self.use_convnext_path and stage_id in [0, 1]:
            hw_shape = (H, W)
            x_nchw = nlc_to_nchw(x, hw_shape)
            convnext_out = self.convnext_path(x_nchw)
            convnext_out_nlc = nchw_to_nlc(convnext_out)
            x = x + convnext_out_nlc  # 特征融合
...
class Head(BaseModule):
    def __init__(self, dim, head_conv, init_cfg=None):
        ...
        stem = [nn.Conv2d(32, 64, head_conv, 2, padding=1 if head_conv==3 else 3, bias=False), nn.BatchNorm2d(64), nn.ReLU(True)]
    ...
    def __init__(self, 
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Block(
                ...
                use_convnext_path=self.use_convnext_path,
                init_cfg=None)
